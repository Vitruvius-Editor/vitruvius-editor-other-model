...
export const beforeEach = async (): Promise<void> => {
    await runSudoCommand("docker-compose -f compose-tests.yml up -d --force-recreate");
...
async function waitForHealth(url: string, timeout: number = 30000, interval: number = 5000): Promise<void> {
