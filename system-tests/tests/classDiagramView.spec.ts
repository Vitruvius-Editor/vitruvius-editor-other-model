...
test('Test class diagram deleting', async ({ page }) => {
  ...
  await page.getByText('Vitruvius Refresh Project', { exact: true }).click();
  await page.getByRole('option', { name: 'ClassDiagram examplePackage' }).locator('a').click();
  await expect(page.locator('[id="packagediagramwidget\\:packagediagramwidget"]')).toContainText('Class1 +myIntAttribute:int - +myOperation(param1:int - ):int -');
  await expect(page.locator('[id="packagediagramwidget\\:packagediagramwidget"]')).toContainText('Class2 +myIntAttribute:Object -');
});
