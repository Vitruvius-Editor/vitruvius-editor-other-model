...
export class TableWidget extends VisualisationWidget<Table> {
          ...
          <thead>
            <tr>
              {this.content.columns
                .filter((column) => column.shouldBeDisplayed)
                .map((column) => (
                ...
                {this.content.columns
                  ...
                  .map((column) => {
                    if (column.editable) {
                      return (
                        <td>
                          <input
                            ...
                            defaultValue={row[
                              column.fieldName as keyof RowEntry
                            ].toString()}
                            onChange={(event) =>
                              this.handleChange(
                                event,
                      ...
                      return (
                        <td>
                          {row[column.fieldName as keyof RowEntry].toString()}
                        </td>
                      );
                    }
                  })}
  ...
  handleChange(
    ...
    (this.content.rows.find((row) => row.uuid === uuid) as any)[propertyName] =
      event.target.value;
  }
}
