...
import {
  VitruviusRefreshProjectContribution,
  RefreshProjectCommand,
} from "./refreshProjectCommand";
...
describe("VitruviusRefreshProjectContribution", () => {
  ...
  beforeEach(() => {
    ...
    displayViewService = {
      updateDisplayViewContent: jest.fn(),
  ...
  it("should show quick pick items and update the project when executing the command", async () => {
    const widgetData: WidgetData = {
      ...
      widget: {
        getLabel: jest.fn().mockReturnValue("TestWidget"),
        close: jest.fn(),
      },
      connection: { uuid: "test-uuid" },
    } as unknown as WidgetData;
    visualisationWidgetRegistry.getWidgets.mockReturnValue([widgetData]);
    displayViewResolver.getContent.mockResolvedValue({
      visualizerName: "TestVisualizer",
      windows: [],
    });
    displayViewService.updateDisplayViewContent.mockResolvedValue({
      visualizerName: "TestVisualizer",
      windows: [],
    });
    displayViewResolver.getWidget.mockResolvedValue({
      show: jest.fn(),
    } as unknown as VisualisationWidget<any>);
