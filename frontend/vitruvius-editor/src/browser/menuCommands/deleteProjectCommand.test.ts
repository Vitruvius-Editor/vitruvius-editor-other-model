import {CommandRegistry, MessageService, QuickPickItem, QuickPickService} from "@theia/core";
...
import { VitruviusDeleteProjectContribution, DeleteProjectCommand } from "./deleteProjectCommand";
...
describe("VitruviusDeleteProjectContribution", () => {
    ...
    beforeEach(() => {
        ...
        connectionService = { getConnections: jest.fn(), deleteConnection: jest.fn() } as unknown as jest.Mocked<ConnectionService>;
    ...
    it("should register the DeleteProjectCommand", () => {
        ...
        expect(commandRegistry.registerCommand).toHaveBeenCalledWith(DeleteProjectCommand, expect.any(Object));
    });
    ...
    it("should show quick pick items and delete the project when executing the command", async () => {
        ...
        visualisationWidgetRegistry.getWidgetsByConnection.mockReturnValue([{ widget: { close: jest.fn() } as unknown as VisualisationWidget<any> } as unknown as WidgetData]);
        ...
        expect(connectionService.deleteConnection).toHaveBeenCalledWith("test-uuid");
    });
    ...
    it("should show an error message if there is an exception while retrieving connections", async () => {
        ...
        expect(messageService.error).toHaveBeenCalledWith("Couldn't connect to the Backend.");
    });
