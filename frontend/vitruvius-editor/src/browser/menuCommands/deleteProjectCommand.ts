import {Command, CommandContribution, CommandRegistry, MessageService, QuickPickService} from "@theia/core";
            ...
            execute: () => this.connectionService.getConnections().then(connections => {
                ...
                let items = connections.map(connection => {
                    return {
                        ...
                        execute: () => {
                            this.connectionService.deleteConnection(connection.uuid).then(() => {
                            ...
                            }).catch(_err => this.messageService.error("Couldn't connect to the Backend server."));
                        }
                    }
                });
