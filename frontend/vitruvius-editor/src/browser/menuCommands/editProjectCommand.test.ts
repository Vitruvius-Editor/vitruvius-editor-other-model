...
import {
  VitruviusEditProjectContribution,
  EditProjectCommand,
} from "./editProjectCommand";
...
describe("VitruviusEditProjectContribution", () => {
  ...
  beforeEach(() => {
    ...
    quickInputService = {
      input: jest.fn(),
    } as unknown as jest.Mocked<QuickInputService>;
    quickPickService = {
      show: jest.fn(),
  ...
  it("should show quick pick items and update the project when executing the command", async () => {
    ...
    connectionService.updateConnection.mockResolvedValue({
      uuid: "test-uuid",
      name: "NewTestProject",
      description: "NewTestDescription",
      url: "http://newtest.url",
      port: 1245,
    });
