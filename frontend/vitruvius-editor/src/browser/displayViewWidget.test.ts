...
describe("DisplayViewWidget", () => {
    ...
    beforeEach(() => {
        ...
        displayViewResolver = { getWidget: jest.fn() } as unknown as jest.Mocked<DisplayViewResolver>;
    ...
    it("should handle project load failure", async () => {
        displayViewService.getDisplayViews.mockRejectedValue(new Error("Connection error"));
        await widget.loadProject(connection);
        await waitFor(() => expect(displayViewService.getDisplayViews).toHaveBeenCalled());
        expect(messageService.error).toHaveBeenCalledWith("Couldn't connect to the given Vitruvius server.");
        expect(widget.getConnection()).toBeNull();
        expect(widget["widgetItems"]).toEqual([]);
    });
    ...
    it("should render the widget with no project loaded", () => {
        ...
        expect(getByText("Currently no Vitruvius project is loaded.")).toBeInTheDocument();
    });
    ...
    it("should render the widget with project loaded", async () => {
        ...
        await widget.loadProject(connection);
        const { getByText } = render(widget.render());
        expect(getByText("The following views are avaliable for the loaded project:")).toBeInTheDocument();
    ...
    it("should handle widget item click and load windows", async () => {
        ...
        expect(displayViewService.getDisplayViewWindows).toHaveBeenCalledWith(connection.uuid, displayView.name);
    });

    it("should handle window click and show content in new widget", async () => {
        displayViewService.getDisplayViews.mockResolvedValue([displayView]);
        displayViewService.getDisplayViewWindows.mockResolvedValue(["Window1"]);
        displayViewService.getDisplayViewContent.mockResolvedValue({ visualizerName: "TextVisualizer", windows: [{ name: "Window1", content: "Content" }] });
        ...
        fireEvent.click(getByText(displayView.name));
        await waitFor(() => expect(widget["widgetItems"][0].windows).toEqual(["Window1"]));
        getByText = render(widget.render()).getByText;
        fireEvent.click(getByText("Window1"));
        expect(displayViewService.getDisplayViewContent).toHaveBeenCalledWith(connection.uuid, displayView.name, { windows: ["Window1"] });
        await waitFor(() => expect(displayViewResolver.getWidget).toHaveBeenCalled());
        expect(visualisationWidgetRegistry.registerWidget).toHaveBeenCalledWith(mockWidget, displayView, connection);
    ...
    it("should store and restore state", () => {
        ...
        expect(state).toEqual({ connection, widgetItems: [{ displayView, windows: ["Window1"] }] });

        const newWidget = new DisplayViewWidget();
        newWidget.restoreState(state);
        expect(newWidget.getConnection()).toEqual(connection);
        expect(newWidget["widgetItems"]).toEqual([{ displayView, windows: ["Window1"] }]);
    });
});